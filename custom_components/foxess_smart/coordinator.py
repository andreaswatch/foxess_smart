...
class FoxESSUpdateCoordinator(DataUpdateCoordinator):
    ...
    def _poll_inverter(self) -> dict:
        ...
        data["grid_current_r"] = round(decode_s32_le(grid_vi[3:5]) * 0.001, 3)
        data["grid_current_s"] = round(decode_s32_le(grid_vi[5:7]) * 0.001, 3)
        data["grid_current_t"] = round(decode_s32_le(grid_vi[7:9]) * 0.001, 3)

        # Block 5: Grid CT Power (39168 - 39169)
        grid_ct = self.client.read_registers(39168, 2)
        grid_ct_val = round(decode_s32_le(grid_ct) * 0.001, 3)  # W -> kW
        ...
        data["load_power_r"] = round(decode_s32_le(load_p[0:2]) * 0.001, 3)
        data["load_power_s"] = round(decode_s32_le(load_p[2:4]) * 0.001, 3)
        data["load_power_t"] = round(decode_s32_le(load_p[4:6]) * 0.001, 3)
        data["load_power_total"] = round(decode_s32_le(load_p[6:8]) * 0.001, 3)

        # Block 7: Battery Combined Power (39237 - 39238)
        bat_p = self.client.read_registers(39237, 2)
        bat_p_val = round(decode_s32_le(bat_p) * 0.001, 3)  # W -> kW
        ...
        data["grid_power_r"] = round(decode_s32_le(grid_p[0:2]) * 0.001, 3)
        data["grid_power_s"] = round(decode_s32_le(grid_p[2:4]) * 0.001, 3)
        data["grid_power_t"] = round(decode_s32_le(grid_p[4:6]) * 0.001, 3)

        # Block 9: PV Power (39279 - 39282)
        pv_p = self.client.read_registers(39279, 4)
        data["pv1_power"] = round(decode_s32_le(pv_p[0:2]) * 0.001, 3)
        data["pv2_power"] = round(decode_s32_le(pv_p[2:4]) * 0.001, 3)
